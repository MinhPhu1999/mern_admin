...
export const addUser = (email, password, name , is_admin) => async (dispatch, getState) => {
    ...
    try {
        //res = 
        await axios.post('http://localhost:8080/admin/adduser', {
            email: email,
            name: name,
            password: password,
            is_admin: is_admin
...
export const updateUser = (email, name, is_admin) => async (dispatch, getState) => {
    ...
    try {
        //res = 
        await axios.post('http://localhost:8080/admin/updateuser', {
            email: email,
            name: name,
            is_admin: is_admin
        })
...
export const deleteUser = (email) => async (dispatch, getState) => {
    ...
    try {
        //res = 
        await axios.post('http://localhost:8080/admin/deleteuser',{
            email: email
        })
    }
    catch (err) {
